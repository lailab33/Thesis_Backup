...
#seeing where the keywords of ecological validity are found for each article
  ...
  #SELF NOTE: THIS NEEDS TO STAY BEOFRE THE SPLIT OF THE REFERENCES AND KEYWORDS
...
#ids: 1025, 1332, 1342 & 1980 were corrected by hand after exporting the csv file to look over personally (all had keywords in the abstract)
  #id: 1025, uses [" ecological" validity]
  #id: 1332 & 1342, uses [(ecological) validity]
  #id 1980, usues [ecological-validity]
...
ggplot(scopus.psych) +
  geom_line(aes(x=year, y=articles)) +
  scale_x_continuous(breaks = c(1970, 1975, 1980, 1985, 1990, 1995, 2000, 2005, 2010, 2015, 2020)) +
  #scale_y_continuous(breaks = c(0, 25, 50, 75, 100, 125, 150, 175, 200, 225, 250)) +
...
data.location.labels <- c("Just Keywords", "Just Abstract", "Abstract and Keywords", "Just Title", "Title and Keywords", "Title and Abstract", "Title, Abstract, and Keywords")

data.location.bar <- cbind(data.location, data.location.labels)

ggplot(data.location.bar, aes(x = n, y = data.location.labels)) +
  geom_col()  +
  labs(title= "Where EV keywords are in the dataset", x = "Number of articles", y = "Location of keywords") +
  theme_thesis

#POTENTIALLY ORDER BY SIZE
...
grand.total.scopus <- scopus.journal.counts %>%
  filter_all(all_vars(.! == "journal"))

```{r}

scopus.journal.counts <- scopus.journal.counts %>%
...
grand.total.scopus <- as.data.frame(t(grand.total.scopus))
...
#deleting columns that aren't about the year

grand.total.scopus <- scopus.journal.counts[-c(1, 82), ]

glimpse(grand.total.scopus)
